...
def handle_cd(current_working_directory, new_working_directory):
    ...
    if len(commands) != 2:
        ...
        try:
            ...
            error = "error: you do not have permissions to change to {0}".format(
                commands[1])
...
def handle_ul(current_working_directory, file_name, service_socket, eof_token):
    ...
    if len(commands) != 2:
        service_socket.sendall(
            pack_msg('error: invalid ul command: ' + file_name, eof_token))
        return

    # response the file name
    service_socket.sendall(
        pack_msg('server received the file name: ' + commands[1], eof_token))
    print(
        f'start receiving file:{commands[1]} from {str(service_socket.getpeername())}')

    # receive the file content
    data = receive_message_ending_with_token(
        service_socket, BUFFER_SIZE, eof_token)
    ...
    service_socket.sendall(
        pack_msg(get_working_directory_info(current_working_directory), eof_token))
...
def handle_dl(current_working_directory, file_name, service_socket, eof_token):
    ...
    if len(args) != 2:
        service_socket.sendall(
            pack_msg('error: invalid dl command: ' + file_name, eof_token))
        return

    print(
        f'received from {str(service_socket.getpeername())} requesting file: {args[1]}')
    ...
    try:
        ...
        error = 'error: could not open/read file ' + \
            args[1] + ', error:' + str(e)
    ...
    # response
    resp = receive_message_ending_with_token(
        service_socket, BUFFER_SIZE, eof_token)
    # sending the file
    ...
    resp = receive_message_ending_with_token(
        service_socket, BUFFER_SIZE, eof_token)
    print(
        f'received from {str(service_socket.getpeername())} file transmission: {resp.decode()}')
    # send current dir info
    service_socket.sendall(
        pack_msg(get_working_directory_info(current_working_directory), eof_token))
...
class ClientThread(Thread):
    ...
    def run(self):
        ...
        self.service_socket.sendall(self.pack_msg(
            get_working_directory_info(self.cwd)))
        ...
        while True:
            ...
            if request == 'exit':
                ...
                handle_ul(self.cwd, request,
                          self.service_socket, self.eof_token)
            elif request.startswith('dl'):
                handle_dl(self.cwd, request,
                          self.service_socket, self.eof_token)
            else:
                self.service_socket.sendall(
                    self.pack_msg('unknown command:' + request))
        ...
        while True:
            try:
                ...
                p = Process(target=task, args=(
                    c_socket, c_address), daemon=True)
                p.start()
